...
def mean_data(file, sheet, group, cols): #avg refers to the variable which will be averaged
...
def chl_0(file, sheet):
    data = pd.read_excel(file, sheet_name=sheet)
    value = data['Chlorophyll, ug/L'].values[0]
    return value


x = chl_0('C:\\Users\\Luis\\Research\\Chlorophyll\\Chlorophyll_Data.xlsx', 'Experiment_4')
...
print(avg_data)
avg_data['Apparent growth rate day'] = 2*np.log((avg_data['Chlorophyll, ug/L']/(x*avg_data['Fraction_whole_seawater'])))



plt.scatter(avg_data['Fraction_whole_seawater'], avg_data['Apparent growth rate day'], )
plt.show()
