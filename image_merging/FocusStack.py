...
def align_images(images):
    ...
    middle = int(len(images) / 2)  # = int(2.5) = 2
    outimages.append(images[3])
    image1gray = cv2.cvtColor(images[3],cv2.COLOR_BGR2GRAY)
    ...
    for i in range(1,len(images)):
        ...
        if use_sift:
            ...
            pairMatches = bf.knnMatch(image_i_desc,image_1_desc, k=2) #k originally was 2
            rawMatches = []
        ...
        newimage = cv2.warpPerspective(images[i], hom, (images[i].shape[1], images[i].shape[0]), flags=cv2.INTER_NEAREST) #was originall INTER_LINEAR
